...
//			return "modules/cms/front/themes/"+category.getSite().getTheme()+"/"+getTpl(article);
	...
	public String getUserByToken(String type, String token, HttpServletRequest request) {

		String GET_URL = "http://sso.yihewanggpo.com/TokenService.asmx/TokenGetCredence?tokenValue=" + token;
...
//			{"orgid":"姚君国","isactive":"true","ssotoken":"9f1c9733-566b-4ca4-ab48-d3935d8149b4","loginid":"admin","roleid":"供应商"}
			
			
			JSONObject obj = new JSONObject(sb.toString());  
			request.getSession().setAttribute("LOGINUSER", obj.getString("loginid"));
			request.getSession().setAttribute("token", obj.getString("ssotoken"));
			     
			list = new ArrayList<>();
			list.add(obj.getString("loginid"));
	...
	public void logoutUser(HttpServletRequest request,HttpServletResponse response,String token  ) {
		...
		for(Cookie c :cookies ){  
		    ...
		    c.setDomain("yihewanggpo.com");  
		    c.setPath("/");  
		    c.setMaxAge(0);  
		    response.addCookie(c);  
		}  
		String GET_URL = "http://sso.yihewanggpo.com/TokenService.asmx/ClearToken?tokenValue=" + token;
		try {
...
//		
	...
	@ResponseBody
	@RequestMapping(value = "fkcontent")
	public String wentifankui(HttpServletRequest request,HttpServletResponse response,String phone,String fkcontent  ) {
			CmsFkcontent  cmsfkcontent =null;
			Date date = new Date();
			
			
			try {
				cmsfkcontent = new CmsFkcontent();
				cmsfkcontent.setPhone(phone);
				cmsfkcontent.setContent(fkcontent);
				User  user = UserUtils.get("1");
				cmsfkcontent.setCreateDate(date);
				cmsfkcontent.setUpdateDate(date);
				cmsfkcontent.setCreateBy(user);
				cmsfkcontent.setUpdateBy(user);
				cmsfkcontent.setDelFlag("0");
